...
class DremioSalesQueryTool(BaseTool):
    ...
    description: str = (
        ...
        "SEMANA FECHADA (ultima semana / semana passada): NUNCA use INTERVAL — "
        "calcule as datas exatas da semana encerrada (segunda a domingo) e use BETWEEN 'AAAA-MM-DD' AND 'AAAA-MM-DD' com datas literais. "
...
class DremioMetasQueryTool(BaseTool):
    ...
    description: str = (
        ...
        "COMO USAR PARA 'VENDAS VS META', 'FATURAMENTO VS META', 'REL VS META', 'REAL VS META': use uma CTE juntando fSales e dMetas_Casas. "
        "ATENCAO JOIN: em fSales o campo casa_ajustado usa o nome COMPLETO da casa (ex: 'Nino BH'), igual ao dMetas_Casas — o JOIN direto funciona. "
        "Padrao CTE para receita vs meta por casa (inclui delta e atingimento): "
        ...
        "Para 'abaixo da meta' ou 'ficamos abaixo': adicione WHERE v.realizado < m.meta no SELECT final. "
        "Para 'acima da meta': adicione WHERE v.realizado > m.meta no SELECT final. "
        "Para filtrar por alavanca/BU (ex: 'nos bares'): adicione WHERE m.alavanca = 'Bar' (ou 'Restaurante' ou 'Iraja') tanto na CTE vendas quanto na CTE metas. "
        "SINTAXE DE DATAS no Dremio: use DATE_SUB(CURRENT_DATE, 1) (ontem), "
        "DATE_TRUNC('month', CURRENT_DATE) (início do mês). "
        "NUNCA use CURRENT_DATE - INTERVAL '1 day' nem CURRENT_DATE - 1. "
        "SEMANA FECHADA (ultima semana / semana passada): NUNCA use INTERVAL — "
