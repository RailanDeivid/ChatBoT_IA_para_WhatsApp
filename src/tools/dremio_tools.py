...
class DremioSalesQueryTool(BaseTool):
    ...
    description: str = (
        ...
        "por ano → TO_CHAR(CAST(data_evento AS DATE), 'YYYY') AS ano. "
        ...
        "CAST() — use SOMENTE no SELECT. NUNCA repita CAST() no WHERE nem no GROUP BY. "
        "Quando precisar filtrar por coluna com CAST(), encapsule em subquery e filtre pelo alias: "
        "SELECT * FROM (SELECT TO_CHAR(CAST(data_evento AS DATE), 'YYYY') AS ano, casa_ajustado, SUM(distribuicao_pessoas) AS fluxo FROM tabela GROUP BY 1, 2) WHERE ano IN ('2023','2024') ORDER BY ano, casa_ajustado. "
        "No GROUP BY: use SEMPRE posicoes ordinais (1, 2, 3...) — NUNCA repita expressoes CAST(). "
...
class DremioDeliveryQueryTool(BaseTool):
    ...
    description: str = (
        ...
        "CAST() — use SOMENTE no SELECT. NUNCA repita CAST() no WHERE nem no GROUP BY. "
        "Quando precisar filtrar por coluna com CAST(), encapsule em subquery e filtre pelo alias: "
        "SELECT * FROM (SELECT CAST(data_evento AS DATE) AS data, ... GROUP BY 1, 2, ...) WHERE data BETWEEN 'AAAA-MM-DD' AND 'AAAA-MM-DD'. "
        "No GROUP BY: use SEMPRE posicoes ordinais (1, 2, 3...) — NUNCA repita expressoes CAST(). "
...
class DremioEstornosQueryTool(BaseTool):
    ...
    description: str = (
        ...
        "casa_ajustado (TEXT, nome da CASA/estabelecimento), "
        "alavanca (TEXT, vertical/segmento. Valores: Bar, Restaurante, iraja), "
        "data_evento (TIMESTAMP, data do estorno — use CAST(data_evento AS DATE) para filtrar), "
...
class DremioMetasQueryTool(BaseTool):
    ...
    description: str = (
        ...
        "Colunas disponíveis: "
        "DATA (TIMESTAMP, data da meta — use CAST(DATA AS DATE) para filtrar), "
        ...
        "CAST() — use SOMENTE no SELECT. NUNCA repita CAST() no WHERE nem no GROUP BY. "
        "Quando precisar filtrar por coluna com CAST(), encapsule em subquery e filtre pelo alias: "
        "SELECT * FROM (SELECT CAST(DATA AS DATE) AS data, ... GROUP BY 1, 2, ...) WHERE data BETWEEN 'AAAA-MM-DD' AND 'AAAA-MM-DD'. "
        "No GROUP BY: use SEMPRE posicoes ordinais (1, 2, 3...) — NUNCA repita expressoes CAST(). "
        "OBRIGATORIO: SQL 100% valido para Dremio. "
        "Input: query SQL valida para Dremio."
    )
