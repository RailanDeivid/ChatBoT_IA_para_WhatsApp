...
class CaptureAgentCallback(BaseCallbackHandler):
    def __init__(self):
        self.action_inputs = set()  # usar set para evitar duplicatas
        self.final_answer = None

    def on_agent_action(self, action, **kwargs):
        self.action_inputs.add(action.tool_input)  # adiciona query única

    def on_agent_finish(self, finish, **kwargs):
        self.final_answer = finish.return_values.get("output")

callback_handler = CaptureAgentCallback()
...
async def webhook(request: Request):
    ...
    if chat_id and message and "@g.us" not in chat_id:
        if chat_id not in conversas:
            ...
            try:
                callback_handler.action_inputs.clear()
                callback_handler.final_answer = None

                prompt = prompt_template.format(q=message)
                output = agent_executor.invoke({"input": prompt})

                queries = "\n\n".join(callback_handler.action_inputs) or "Nenhuma consulta SQL executada."
                resposta_final = callback_handler.final_answer or "Desculpe, não consegui responder sua pergunta no momento."

                resposta_completa = f"Consulta SQL executada:\n{queries}\n\nResposta:\n{resposta_final}"
