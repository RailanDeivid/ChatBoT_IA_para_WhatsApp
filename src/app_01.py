...
from langchain_openai import ChatOpenAI
from dotenv import load_dotenv
from src.config import STRING_CONEXAO, OPENAI_MODEL_NAME, OPENAI_MODEL_TEMPERATURE,AI_SYSTEM_PROMPT
...
model = ChatOpenAI(model=OPENAI_MODEL_NAME, temperature=OPENAI_MODEL_TEMPERATURE)
db = SQLDatabase.from_uri(STRING_CONEXAO)
toolkit = SQLDatabaseToolkit(db=db, llm=model)
system_message = hub.pull('hwchase17/react')
...
prompt_template = PromptTemplate.from_template(AI_SYSTEM_PROMPT)
